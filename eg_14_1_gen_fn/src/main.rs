/**
 * @Author: 白银
 * @Date: 2022-08-27 21:00:34
 * @LastEditTime: 2022-08-27 21:01:44
 * @LastEditors: 白银
 * @Description: 调用函数时，使用显式指定的类型参数会像是这样：fun::<A, B, ...>() https://rustwiki.org/zh-CN/rust-by-example/generics/gen_fn.html
 */
...
// `SGen<>` 显式地接受了类型参数 `A`，且在 `gen_spec_t` 中，`A` 没有被用作泛型类型参数，所以函数不是泛型的
